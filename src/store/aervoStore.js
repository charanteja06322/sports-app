...
if (typeof window !== "undefined" && window.BroadcastChannel) {
  try {
    broadcastChannel = new BroadcastChannel("aervo_live_channel");
  } catch {
    // fallback
  }
}

export const useAervoStore = create((set, get) => ({
  ...
  switchAthlete: (athlete) => {
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem("aervo_active_user_id", String(athlete.id));
    }
  },
  ...
  fetchLiveData: async () => {
    try {
      ...
      if (res.ok) {
        ...
        const savedId = typeof window !== "undefined" ? localStorage.getItem("aervo_active_user_id") : null;
        const currentMe = me 
      ...
      console.warn("Could not sync with /api/feed:", e.message);
    }
  },
}));
...
if (typeof window !== "undefined") {
  try {
    localStorage.removeItem("aervo_state_v1");
  } catch {}

  // Listen to broadcast from other tabs
  if (broadcastChannel) {
    broadcastChannel.onmessage = () => {
      useAervoStore.getState().fetchLiveData();
    };
  }
  ...
  // Background polling every 2.5 seconds for multi-device/multi-browser sync
  setInterval(() => {
    useAervoStore.getState().fetchLiveData();
  }, 2500);
}
