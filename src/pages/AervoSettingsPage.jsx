...
export default function AervoSettingsPage() {
  ...
  const createAthlete = useAervoStore((s) => s.createAthlete);
  const clearAllData = useAervoStore((s) => s.clearAllData);

  const activeSport = useAervoStore((s) => s.activeSport);
  const setSport = useAervoStore((s) => s.setSport);
  const updateProfile = useAervoStore((s) => s.updateProfile);
  ...
  const handleCreateAthlete = async (e) => {
    ...
    await createAthlete({
      ...
      bio: newAthleteBio.trim() || "Multi-sport athlete on Aervo",
      avatarUrl: "",
  ...
  return (
    <main className="mx-auto max-w-[880px] px-5 pb-16 pt-8 sm:px-8 lg:px-10 lg:pt-12">
      <PageHeader
        ...
        body="Manage your verified Aervo athlete identity, public Player ID, active sporting perspective, and multi-user testing."
      />
      ...
      {createModalOpen && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4 backdrop-blur-xs">
          <form
            ...
            className="aervo-fade w-full max-w-[460px] rounded-3xl border border-[#DDD6C8] bg-white p-6 shadow-2xl sm:p-8"
          >
